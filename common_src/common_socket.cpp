...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        s = connect(socket_fd, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
    ...
    /*
     * Si el `socket_fd` es -1 es por q (1) no entramos nunca al for-loop
     * o (2) la ultima llamada `socket` fallo y retorno -1.
     * En ambos casos no debemos cerrar ningun socket.
     *
     * Si en cambio `socket_fd` es distinto de -1 significa q tenemos
     * un socket abierto.
     * */
    if (socket_fd != -1)
        ::close(socket_fd);
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(socket_fd, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket Socket::accept() {
     ...
     *
     * Por eso creamos un `Socket` lo pasamos por movimiento
     * */
